...
const Banner = () => {
  ...
  return (
    <>
      <section style={headerStyle} className="h-95 sm:h-95 w-full rounded-sm shadow relative overflow-hidden">
        <Slider {...settings}>
          {banners.map((el, i) => (
            <img draggable="false" className="h-95 sm:h-95 w-full object-cover" src={el} alt="banner" key={i} />
          ))}
        </Slider>
      </section>
    </>
